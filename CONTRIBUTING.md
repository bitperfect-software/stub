...
Node **22+** for development (the published package supports **>= 20.19**). There is no watch-mode
build: `npm run dev` runs the TypeScript sources directly through `tsx`.

| Script | |
|---|---|
| `npm run dev -- <args>` | Run the CLI from source. |
| `npm run build` | Bundle to `dist/index.js` with esbuild, shebang and executable bit included. |
| `npm test` | The whole suite — unit and end-to-end. |
| `npm run test:unit` / `test:watch` | Just the unit project; the watch variant for a tight loop. |
| `npm run coverage` | Unit coverage against the thresholds in `vitest.config.ts`. |
| `npm run typecheck` / `lint` / `format` | Individually. `lint:fix` and `format` write. |
| **`npm run check:all`** | **The gate: typecheck, lint, format check, tests. Keep it green.** |

## Verifying a change

Run `npm run check:all`. It is the whole story — there is no manual checklist any more.
...
- `npm run check:all` green before you push.
...
```bash
npm version <patch|minor|major>   # commit + v-prefixed tag, matching package.json
...
`test/` — out of the tarball; check with `npm pack --dry-run` if you change it.
...
|---|---|---|
| `ci.yml` | push to `main`, every pull request | `check` runs the gate plus the build on Node 22 and 24. `package` packs the tarball, installs it globally and runs the CLI on Linux and Windows, on the `engines` floor and on current. |
| `docs.yml` | push to `main` touching `docs/**` | Uploads `docs/` verbatim as a Pages artifact and deploys it. |
| `release.yml` | a `v*` tag | Above. |

The `package` job is the one that matters. It is the only place that catches a missing entry in
...
Workflow YAML is inside the Prettier gate (`.github/**/*.yml`), so run `npm run format` after editing
